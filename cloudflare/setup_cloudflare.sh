...
EOL
    ...
    echo -e "\n${YELLOW}${BOLD}CloudFlare Tunnel Setup Instructions (2025)${NC}"
    echo -e "${YELLOW}=====================================================================${NC}"
    echo -e "To create a tunnel and get a token, follow these detailed steps:"
    echo -e "\n${CYAN}1. Go directly to CloudFlare Zero Trust dashboard:${NC}"
    echo -e "   https://one.dash.cloudflare.com/"
    echo -e "\n${CYAN}2. In the CloudFlare Zero Trust dashboard:${NC}"
    echo -e "   - Click on 'Tunnels' in the left sidebar"
    ...
    echo -e "   - Select 'Manual' as the installation method"
    echo -e "   - Copy the tunnel token shown in the interface"
    echo -e "   - The token is a long alphanumeric string needed for this setup"
